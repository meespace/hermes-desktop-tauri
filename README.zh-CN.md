...
这是一个将 Hermes Desktop 从 Electron 迁移到 Tauri 的社区仓库。
...
欢迎提 issue、提 PR、做对照测试，也欢迎专门帮忙做 Electron 与 Tauri 的逐页验收。开始之前，建议先看 [CONTRIBUTING.md](./CONTRIBUTING.md) 和 [MIGRATION_PLAN.md](./MIGRATION_PLAN.md)。

## 仓库首发说明

如果你准备把这份目录直接作为公开仓库首发，建议先看 [OPEN_SOURCE_RELEASE_CHECKLIST.zh-CN.md](./OPEN_SOURCE_RELEASE_CHECKLIST.zh-CN.md)。

## 行为准则

请把这里当成一个认真做事、但也讲人情味的仓库。讨论问题可以直接，表达方式请克制。完整说明见 [CODE_OF_CONDUCT.md](./CODE_OF_CONDUCT.md)。
