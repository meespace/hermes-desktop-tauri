...
async fn spawn_local_hermes(
    ...
    let hermes_cli_info = probe_hermes_cli_info();
    let hermes_command = if let Some(command) = resolve_hermes_cli_binary(&active_root) {
        command.to_string_lossy().to_string()
    } else {
        let install_command = hermes_cli_install_command_for(std::env::consts::OS);
        emit_bootstrap_event(
            app,
            &*state,
            missing_hermes_cli_payload(std::env::consts::OS, &active_root),
        );
        return Err(format!(
    ...
    let mut command = desktop_command(&hermes_command);
    command
    ...
    for (key, value) in update_command_env(&active_root) {
        command.env(key, value);
    }
    if let Some(web_dist) = hermes_cli_info
        .as_ref()
        .and_then(|info| info.project_root.as_ref())
        .and_then(|root| resolve_hermes_web_dist_dir(root))
    {
        command.env("HERMES_WEB_DIST", web_dist);
    }
        ...
        if let Ok(resp) = client.get(format!("{}/api/status", base_url)).send().await {
            if resp.status().is_success() {
                if !probe_desktop_dashboard_routes(&client, &base_url).await {
                    ...
                    let _ = child.wait();
                    return Err(format!(
                        "Hermes dashboard started on {}, but it is missing desktop routes. Upgrade Hermes Agent and restart Hermes Desktop Community. Log: {}",
                        base_url,
                        desktop_log_path().to_string_lossy()
                    ));
                }
...
pub async fn check_hermes_agent_update() -> Result<serde_json::Value, String> {
    let config = read_update_source_config();
    let status = inspect_system_hermes_status(&config)?;

    Ok(system_hermes_status_payload(
        &config,
        status.installed,
        status.branch,
        status.current_commit,
...
#[tauri::command]
pub async fn install_hermes_agent(app: AppHandle) -> Result<serde_json::Value, String> {
    let config = read_update_source_config();
    Ok(manual_hermes_agent_action_payload(
        &app,
        &config,
        "Hermes Desktop Community no longer installs a separate managed Hermes. Run the copied command to install or upgrade your system Hermes CLI, then retry.".to_string(),
    ))
}
...
pub async fn update_hermes_agent(app: AppHandle) -> Result<serde_json::Value, String> {
    let config = read_update_source_config();
    let hermes = resolve_hermes_cli_binary(&active_hermes_root()).ok_or_else(|| {
        "Hermes CLI was not found on PATH. Install Hermes manually, then retry the local check."
            .to_string()
    })?;
    let update_root =
        sync_system_hermes_update_source(&config)?.unwrap_or_else(resolve_update_root);
    let env = update_command_env(&update_root);
    ...
    let updated = run_streamed_update(
        &hermes,
        &["update", "--yes"],
        &update_root,
        "update",
    ...
    if updated != 0 {
        emit_update_progress(
            ...
            serde_json::json!({
                "stage": "error",
                "message": "Hermes CLI update failed.",
                "percent": serde_json::Value::Null,
                "error": "update-failed",
                "at": chrono::Utc::now().timestamp_millis()
            }),
        );
        return Err("Hermes CLI update failed.".to_string());
    }
    ...
    let status = inspect_system_hermes_status(&config).unwrap_or(SystemHermesStatusSnapshot {
        ...
        latest_version: None,
        update_available: false,
        message: "Hermes CLI upgraded successfully.".to_string(),
    });
    let mut payload = system_hermes_status_payload(
        &config,
        status.installed,
        ...
        status.latest_version,
        status.update_available,
        "Hermes CLI upgraded successfully.".to_string(),
    );
    if let Some(object) = payload.as_object_mut() {
        object.insert("ok".to_string(), serde_json::Value::Bool(true));
    }
...
#[tauri::command]
pub async fn repair_hermes_agent(app: AppHandle) -> Result<serde_json::Value, String> {
    let config = read_update_source_config();
    Ok(manual_hermes_agent_action_payload(
        &app,
        &config,
        "Repair your system Hermes CLI manually with the copied install command, then rerun the local check.".to_string(),
    ))
}
