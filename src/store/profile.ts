...
export function defaultProfileInfo(): ProfileInfo {
  return {
    has_env: false,
    is_default: true,
    model: null,
...
export function profilesWithDefaultFallback(profiles: readonly ProfileInfo[]): ProfileInfo[] {
  if (profiles.some(profile => profile.is_default || normalizeProfileKey(profile.name) === 'default')) {
    return [...profiles]
  }

  return [defaultProfileInfo(), ...profiles]
}

export function setCachedProfiles(profiles: readonly ProfileInfo[]): void {
  const current = $profiles.get()

  if (profiles.length > 0) {
    $profiles.set(profilesWithDefaultFallback(profiles))

    return
  }

  if (current.length === 0) {
    $profiles.set([defaultProfileInfo()])
  }
}
...
export async function switchProfile(name: string): Promise<void> {
  ...
  if (window.hermesDesktop.profile?.set) {
