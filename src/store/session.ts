...
const DEFAULT_PROJECT_DIR_KEY = 'hermes.desktop.default-project-dir'

export const getRememberedWorkspaceCwd = (): string => storedString(WORKSPACE_CWD_KEY)?.trim() || ''
export const getRememberedDefaultProjectDir = (): string => storedString(DEFAULT_PROJECT_DIR_KEY)?.trim() || ''
...
export const mergeWorkingSessions = (
  previous: SessionInfo[],
  incoming: SessionInfo[],
  workingIds: readonly string[]
): SessionInfo[] => mergeSessionPage(previous, incoming, workingIds)
...
export const $defaultProjectDir = atom(getRememberedDefaultProjectDir())
...
export const setDefaultProjectDir = (next: Updater<string>) => {
  updateAtom($defaultProjectDir, next)
  persistString(DEFAULT_PROJECT_DIR_KEY, $defaultProjectDir.get().trim() || null)
}
