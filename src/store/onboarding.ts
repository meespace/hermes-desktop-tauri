...
export function requestDesktopOnboarding(reason = DEFAULT_ONBOARDING_REASON) {
  if (!shouldShowCredentialWarning(reason, providerFromCredentialWarning(reason))) {
    return
  }
...
// One-shot handoff for Settings -> Providers. The settings page can ask the
// shared onboarding overlay to open and immediately start one provider's real
// OAuth flow, instead of duplicating OAuth UI in settings.
...
export async function saveOnboardingApiKey(envKey: string, value: string, label: string, ctx: OnboardingContext) {
  ...
  // Local / custom endpoint selection is a base URL, not an API key.
