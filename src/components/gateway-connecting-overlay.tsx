...
export function GatewayConnectingOverlay() {
  ...
  return (
    <div
      className={cn(
        'fixed inset-0 z-[1200] grid place-items-center bg-[color-mix(in_srgb,var(--background)_88%,var(--surface))] transition-opacity duration-500 ease-out',
        overlayHidden ? 'pointer-events-none opacity-0' : 'opacity-100'
      )}
      data-slot="gateway-connecting-overlay"
    >
      <style>{'@keyframes gco-cursor { 0%, 49% { opacity: 1 } 50%, 100% { opacity: 0 } }'}</style>
      <div
        className={cn(
          'rounded-full border border-[var(--border)] bg-[var(--surface)] px-5 py-3 shadow-[var(--overlay-shadow)] backdrop-blur-sm transition duration-300 ease-out',
          leaving ? 'translate-y-2 opacity-0 saturate-0' : 'translate-y-0 opacity-100 saturate-100'
        )}
        data-slot="gateway-connecting-chip"
      >
        <span className="inline-flex items-center pl-[0.35em] font-mono text-[0.64rem] font-semibold uppercase tracking-[0.4em] tabular-nums text-[var(--accent)]">
          {PREFIX}
          {tail}
          <span
            aria-hidden="true"
            className="dither ml-0.5 inline-block size-2 shrink-0 -translate-y-px rounded-[1px] bg-[var(--accent)]"
