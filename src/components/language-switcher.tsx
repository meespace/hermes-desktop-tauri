...
export function LanguageSwitcher({ className, collapsed = false, dropUp = false }: LanguageSwitcherProps) {
  ...
  const trigger = (
    <Button
      ...
      aria-label={title}
      className={cn(
        'min-w-32 justify-between gap-2 border-[var(--border)] bg-[var(--surface-secondary)] px-2.5 text-left text-[var(--muted)] hover:text-[var(--foreground)]',
    ...
    >
      <span className="inline-flex min-w-0 items-center gap-2">
        ...
        {!collapsed ? <span className="truncate">{current.name}</span> : null}
      </span>
      {!collapsed ? <ChevronDown className="size-3 shrink-0 opacity-70" /> : null}
    </Button>
  )
  ...
  return (
    <Command className="bg-transparent" shouldFilter={false}>
      ...
      <CommandList className="max-h-80 p-1">
        {filtered.length === 0 ? (
          <div className="py-6 text-center text-sm text-[var(--muted)]">{noResults}</div>
        ) : (
          filtered.map(([code, meta]) => {
            ...
            return (
              <CommandItem
                className={cn(selected ? 'font-medium text-[var(--foreground)]' : 'text-[var(--muted)]')}
                disabled={disabled}
                key={code}
                onSelect={() => onSelect(code)}
                value={code}
              >
                <Check className={cn('size-3.5 shrink-0 text-[var(--accent)]', !selected && 'invisible')} />
                <span className="min-w-0 flex-1 truncate">{meta.name}</span>
                <span className="font-mono text-[0.65rem] uppercase text-[var(--muted)]">{code}</span>
              </CommandItem>
            )
          })
        )}
