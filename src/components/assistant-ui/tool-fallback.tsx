...
const TOOL_HEADER_DURATION_CLASS = 'shrink-0 text-[0.625rem] tabular-nums text-[var(--muted)]'

const TOOL_HEADER_SUBTITLE_CLASS =
  'text-[length:var(--conversation-caption-font-size)] leading-(--conversation-caption-line-height) text-[var(--muted)]'
...
const TOOL_SECTION_LABEL_CLASS = 'mb-1 text-[0.65rem] font-medium uppercase tracking-[0.08em] text-[var(--muted)]'
...
const TOOL_SECTION_SURFACE_CLASS =
  'max-h-20 max-w-full overflow-auto bg-transparent px-2 py-1.5 text-[color-mix(in_srgb,var(--foreground)_84%,transparent)]'
...
function ToolGlyph({ icon, status }: { icon?: string; status?: ToolStatus }) {
  const node = status ? (
    ...
    <Codicon className="text-[var(--muted)]" name={icon} size="0.875rem" />
  ) : null
...
function ToolEntry({ part }: ToolEntryProps) {
  ...
  return (
    <div
      className={cn(
        'min-w-0 max-w-full overflow-hidden text-[length:var(--conversation-tool-font-size)] text-[var(--muted)]',
        open && 'rounded-[0.625rem] border border-[var(--border)]'
      )}
      data-slot="tool-block"
      ref={enterRef}
    >
      <div className={cn(open && 'border-b border-[var(--separator)] px-2 py-1.5')}>
        <DisclosureRow
          ...
          <span className="flex min-w-0 items-center gap-1.5">
            ...
            <FadeText
              className={cn(
                ...
                isPending && 'shimmer text-[var(--muted)]',
      ...
      {isPending ? <PendingToolApproval part={part} /> : null}
      {open && (
        <div className="grid w-full min-w-0 max-w-full gap-1.5 overflow-hidden p-1.5">
          ...
          {view.imageUrl && (
            <div className="max-w-72 overflow-hidden rounded-[0.25rem] border border-[var(--border)]">
              <ZoomableImage alt="Tool output" className="h-auto w-full object-cover" src={view.imageUrl} />
            </div>
          )}
          {hasSearchHits && view.searchHits && (
            <div className="max-w-full text-xs leading-relaxed text-[color-mix(in_srgb,var(--foreground)_84%,transparent)]">
                  ...
                  <div className={cn('space-y-0.5', view.stdout && 'mt-1.5')}>
                    ...
                    <pre
                      className={cn(
                        ...
                        'whitespace-pre-wrap wrap-anywhere text-[var(--muted)]'
                      )}
                    >
...
export const ToolGroupSlot: FC<PropsWithChildren<{ endIndex: number; startIndex: number }>> = ({
  ...
  return (
    <ToolEmbedContext.Provider value={false}>
      <div className="grid min-w-0 max-w-full gap-[var(--tool-row-gap,0.125rem)] overflow-hidden" data-slot="tool-block" ref={enterRef}>
        {children}
      </div>
