...
function systemMessage(text: string): ThreadMessage {
  return {
    id: `system:${text}`,
    role: 'system',
    content: [{ type: 'text', text }],
...
function ConversationHarness({
  ...
  return (
    <AssistantRuntimeProvider runtime={runtime}>
      <Thread assistantDisplayName={assistantDisplayName} />
    </AssistantRuntimeProvider>
  )
}
...
describe('assistant-ui streaming renderer', () => {
  ...
  it('renders user messages inside the HeroUI shell', () => {
    const { container } = render(<MessageHarness message={userMessage()} />)

    const stickyRoot = container.querySelector('[data-slot="aui_user-message-root"]') as HTMLElement
    const bubble = container.querySelector('[data-chat-bubble="true"]') as HTMLElement

    expect(stickyRoot.className).toContain('group/user-message')
    expect(bubble.className).toContain('composer-human-message')
    expect(bubble.className).toContain('bg-[color-mix(in_srgb,var(--accent)_7%,var(--surface))]')
    expect(bubble.className).toContain('ring-inset')
  })
  ...
  it('shows speaker aliases only when the speaker changes', () => {
    ...
    const { container } = render(<ConversationHarness assistantDisplayName="Planner" messages={conversation} />)
    const speakers = [...container.querySelectorAll('[data-slot="aui_message-speaker"]')].map(node => node.textContent)

    expect(speakers).toEqual(['You', 'Planner', 'You'])
  })
  ...
  it('renders steer system messages with the official compact status treatment', () => {
    render(<MessageHarness message={systemMessage('steer:focus on the local stack trace')} />)

    expect(screen.getByText('steered')).toBeTruthy()
    expect(screen.getByText('focus on the local stack trace')).toBeTruthy()
  })
