...
const Checkmark: FC<{ status: TodoStatus; label: string }> = ({ status, label }) => {
  ...
  return (
    <Checkbox
      ...
      checked={checked}
      className={cn(
        'size-[1.1rem] shrink-0 rounded-full border-[var(--border)] pointer-events-none disabled:cursor-default disabled:opacity-100',
        checked &&
        ...
        status === 'cancelled' && 'border-[color-mix(in_srgb,var(--foreground)_42%,transparent)]'
      )}
      disabled
    />
  )
...
export const HoistedTodoPanel: FC<{ todos: TodoItem[] }> = ({ todos }) => {
  ...
  return (
    <section
      className="mt-1 mb-3 inline-block w-fit max-w-full overflow-hidden rounded-2xl border border-[var(--border)] bg-[var(--surface)] align-top shadow-[var(--surface-shadow)]"
