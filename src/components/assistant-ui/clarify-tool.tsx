...
function ClarifyToolPending({ args }: ToolCallMessagePartProps) {
  ...
  return (
    <div
      className={cn(
        'mb-3 mt-2 grid gap-3 rounded-xl border border-[var(--border)] bg-[var(--surface)]/88 px-4 py-3.5 text-sm shadow-[var(--field-shadow)]',
    ...
    >
      <div className="flex items-start gap-2.5">
        ...
        <div className="grid flex-1 gap-0.5">
          ...
          <span className="whitespace-pre-wrap leading-snug text-foreground">
            {question || <em className="text-[color-mix(in_srgb,var(--foreground)_58%,transparent)]">Loading question…</em>}
          </span>
        </div>
      </div>
      ...
      {!typing && hasChoices && (
        <div className="grid gap-1.5" onKeyDown={handleChoiceKey} role="group">
          {choices.map((choice, index) => (
            <button
              className={cn(
                'group/choice flex w-full items-center gap-3 rounded-lg border border-[var(--border)] bg-[var(--surface)]/96 px-3 py-2 text-left text-sm text-foreground/95 shadow-[var(--field-shadow)]',
                'transition-colors hover:border-[var(--border)] hover:bg-[var(--surface-secondary)] disabled:cursor-not-allowed disabled:opacity-55'
              )}
              data-choice
              ...
              type="button"
            >
              <span className="grid size-5 shrink-0 place-items-center rounded-md bg-[var(--surface-secondary)] text-[0.6875rem] font-mono tabular-nums text-[var(--muted)] group-hover/choice:bg-[var(--surface)]">
                {index + 1}
              </span>
            ...
            <button
              className={cn(
              'flex w-full items-center gap-3 rounded-lg border border-dashed border-[var(--border)] bg-transparent px-3 py-2 text-left text-sm text-[var(--muted)]',
              'transition-colors hover:border-[var(--border)] hover:bg-[var(--surface-secondary)] hover:text-foreground'
              )}
            disabled={submitting}
            onClick={() => {
              setTyping(true)
          ...
          >
            <span
              aria-hidden
              className="grid size-5 shrink-0 place-items-center rounded-md bg-[var(--surface-secondary)] text-[var(--muted)]"
            >
      ...
      {(typing || !hasChoices) && (
        <form className="grid gap-2" onSubmit={handleSubmitFreeform}>
          <Textarea
            className="min-h-20 resize-y rounded-lg border-[var(--border)] bg-[var(--surface)]/96 text-sm shadow-[var(--field-shadow)]"
            disabled={submitting}
          ...
          <div className="flex items-center justify-between gap-2">
            <span className="text-[0.6875rem] text-[var(--muted)]">⌘/Ctrl + Enter to send</span>
      ...
      {!typing && hasChoices && (
        <div className="flex items-center justify-between text-[0.6875rem] text-[var(--muted)]">
          <span>1–{choices.length} to pick</span>
          <button
            className="bg-transparent text-[var(--muted)] underline-offset-4 decoration-current/20 hover:text-foreground hover:underline disabled:opacity-50"
            disabled={!ready || submitting}
            onClick={() => void respond('')}
            type="button"
          >
