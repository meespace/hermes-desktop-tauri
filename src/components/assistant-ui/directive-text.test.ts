...
describe('hermesDirectiveFormatter.parse', () => {
  ...
  it('parses @session refs and shortens their default label', () => {
    const segments = hermesDirectiveFormatter.parse('continue from @session:default/1234567890abcdef')

    expect(segments).toEqual([
      { kind: 'text', text: 'continue from ' },
      { kind: 'mention', type: 'session', label: '12345678…', id: 'default/1234567890abcdef' }
    ])
  })
})
