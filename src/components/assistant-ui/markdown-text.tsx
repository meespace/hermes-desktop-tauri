...
function OpenMediaButton({ kind, path }: { kind: 'audio' | 'video'; path: string }) {
  return (
    <button
      className="mt-2 bg-transparent text-xs font-medium text-[var(--muted)] underline underline-offset-4 decoration-current/20 hover:text-foreground"
...
function MediaAttachment({ path }: { path: string }) {
  ...
  if (kind === 'audio' && src) {
    return (
      <span className="my-3 block max-w-md rounded-xl border border-[var(--border)] bg-[var(--surface-secondary)]/55 p-3 shadow-[var(--field-shadow)]">
        <span className="mb-2 block truncate text-xs font-medium text-[var(--muted)]">{name}</span>
  ...
  if (kind === 'video' && src) {
    return (
      <span className="my-3 block max-w-2xl rounded-xl border border-[var(--border)] bg-[var(--surface-secondary)]/55 p-3 shadow-[var(--field-shadow)]">
        <span className="mb-2 block truncate text-xs font-medium text-[var(--muted)]">{name}</span>
        <video
    ...
    () =>
      ({
        ...
        hr: ({ className, ...props }: ComponentProps<'hr'>) => (
          <hr className={cn('border-[var(--separator)]', className)} {...props} />
        ),
        blockquote: ({ className, ...props }: ComponentProps<'blockquote'>) => (
          <blockquote
            className={cn('border-l-2 border-[var(--border)] pl-3 text-[var(--muted)] italic', className)}
            {...props}
          />
        ),
        ...
        table: ({ className, ...props }: ComponentProps<'table'>) => (
          <div className="aui-md-table my-2 max-w-full overflow-x-auto rounded-[0.375rem] border border-[var(--border)]">
            <table
              className={cn(
                'm-0 w-full border-collapse text-[0.8125rem] [&_tr]:border-b [&_tr]:border-[var(--separator)] last:[&_tr]:border-0',
                className
              )}
              {...props}
            />
        ...
        thead: ({ className, ...props }: ComponentProps<'thead'>) => (
          <thead className={cn('m-0 bg-[var(--surface-secondary)]/55 text-[var(--muted)]', className)} {...props} />
        ),
        th: ({ className, ...props }: ComponentProps<'th'>) => (
          <th
            className={cn(
              'px-2.5 py-1.5 text-left align-middle text-[0.75rem] font-medium text-[var(--muted)]',
              className
            )}
            {...props}
          />
    ...
    <DeferStreamingText>
      <StreamdownTextPrimitive
        ...
        containerClassName={cn(
          ...
          'prose-li:marker:text-[color-mix(in_srgb,var(--foreground)_58%,transparent)]',
