...
export function ModelPickerDialog({
  ...
  return (
    <Dialog onOpenChange={onOpenChange} open={open}>
      <DialogContent
        className={cn(
          'max-h-[85vh] max-w-2xl gap-0 overflow-hidden bg-[var(--overlay)] p-0 shadow-[var(--overlay-shadow)]',
          contentClassName
        )}
      >
        <DialogHeader className="border-b border-[var(--separator)] bg-[var(--surface)] px-4 py-3">
          <DialogTitle>{t.modelPicker.switchModel}</DialogTitle>
          <DialogDescription className="font-mono text-xs leading-relaxed text-[var(--muted)]">
            current: {optionsModel || currentModel || '(unknown)'}
            {optionsProvider || currentProvider ? ` · ${optionsProvider || currentProvider}` : ''}
          </DialogDescription>
        </DialogHeader>

        <Command className="rounded-none bg-[var(--surface)]" shouldFilter={false}>
          <CommandInput
            autoFocus
          ...
          <CommandList className="max-h-96">
            {!loading && !error && <CommandEmpty>{t.modelPicker.noModels}</CommandEmpty>}
            <ModelResults
        ...
        <DialogFooter className="flex-row items-center justify-between gap-3 border-t border-[var(--separator)] bg-[var(--surface)] p-3 sm:justify-between">
          <label className="flex cursor-pointer select-none items-center gap-2 text-xs text-[var(--muted)]">
            <Checkbox
              checked={persistGlobal || !sessionId}
              disabled={!sessionId}
              onCheckedChange={checked => setPersistGlobal(checked === true)}
            />
            {sessionId ? t.modelPicker.persistGlobalSession : t.modelPicker.persistGlobal}
          </label>
...
function ModelResults({
  ...
  if (error) {
    return (
      <div className="px-3 py-3">
        <InlineNotice kind="error" title={t.modelPicker.couldNotLoad}>
          {error}
        </InlineNotice>
      </div>
    )
  ...
  if (providers.length === 0) {
    return <div className="px-4 py-6 text-sm text-[var(--muted)]">{t.modelPicker.noAuthenticatedProviders}</div>
  }
            ...
            {models.map(model => {
              ...
              return (
                ...
                >
                  ...
                  {locked && <span className="shrink-0 text-[0.62rem] uppercase tracking-wide opacity-80">{t.modelPicker.pro}</span>}
                  <ModelPrice isCurrent={isCurrent} price={price} />
                </CommandItem>
              )
            })}
            {unavailable.size > 0 && (
              <div className="px-6 pb-2 pt-1 text-[0.62rem] leading-relaxed text-[var(--muted)]">
  ...
  return (
    <span
      className={cn(
        ...
        isCurrent ? 'text-primary-foreground/80' : 'text-[var(--muted)]'
      )}
      title={t.modelPicker.inputOutputPrice}
    >
...
function ProviderHeading({ provider }: { provider: ModelOptionProvider }) {
  ...
  return (
    <span className="flex min-w-0 items-center gap-2">
      ...
      <span className="font-mono text-xs font-normal normal-case tracking-normal text-[var(--muted)]">
