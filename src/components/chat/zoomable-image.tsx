  ...
  async function handleDownload() {
    ...
    try {
      if (window.hermesDesktop?.saveImageFromUrl) {
        const saved = await window.hermesDesktop.saveImageFromUrl(src, imageFilename(src))
      ...
      if (isMissingIpcHandler(error)) {
        try {
          ...
          notify({
            ...
            message: 'Restart Hermes Desktop Community to use Save Image.'
          })
        } catch (fallbackError) {
          notifyError(fallbackError, 'Restart Hermes Desktop Community to save images')
        }
...
function ImageActionButton({
  ...
  return (
    <button
      ...
      className={cn(
        'absolute right-2 top-2 grid size-8 place-items-center rounded-full border border-[var(--border)] bg-[var(--surface)]/84 text-[var(--muted)] opacity-0 shadow-[var(--field-shadow)] backdrop-blur transition-opacity hover:bg-[var(--surface-secondary)] hover:text-foreground focus-visible:opacity-100 disabled:opacity-50',
