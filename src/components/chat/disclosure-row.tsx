...
export function DisclosureRow({
  ...
  return (
    <div className="group/disclosure-row relative flex w-full max-w-full min-w-0 text-[var(--muted)]">
      <button
        aria-expanded={onToggle ? open : undefined}
        className={cn(
          // max-w-fit so the click target hugs the title text width — no
          // background fill, just the cursor + the affordance caret.
          'flex min-w-0 max-w-fit items-start gap-1.5 rounded-[calc(var(--radius)*1.25)] border border-transparent px-1.5 py-0.5 text-left transition-[background-color,border-color,color]',
          onToggle
            ? 'cursor-pointer hover:border-[var(--border)] hover:bg-[var(--surface-secondary)] hover:text-foreground focus-visible:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[color-mix(in_srgb,var(--focus)_22%,transparent)]'
            : 'cursor-default'
        )}
        disabled={!onToggle}
        onClick={onToggle}
