...
const DIFF_LINE_KINDS: DiffLineKind[] = [
  ...
  {
    className: 'text-[color-mix(in_srgb,var(--foreground)_58%,transparent)]',
...
export function DiffLines({ className, text, ...props }: DiffLinesProps) {
  return (
    <pre
      className={cn(
        'mt-2 max-h-96 max-w-full min-w-0 overflow-auto rounded-md border border-[var(--border)] bg-[var(--surface-secondary)]/55 px-2.5 py-1.5 font-mono text-[0.7rem] leading-relaxed text-[var(--muted)]',
        className
      )}
