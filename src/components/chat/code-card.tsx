...
function CodeCard({ className, ...props }: React.ComponentProps<'div'>) {
  return (
    <div
      className={cn(
        'min-w-0 max-w-full overflow-hidden rounded-[0.625rem] border border-[var(--border)] text-[length:var(--conversation-tool-font-size)] text-[var(--muted)]',
        className
      )}
      data-slot="code-card"
      {...props}
...
function CodeCardHeader({ className, ...props }: React.ComponentProps<'div'>) {
  return (
    <div
      className={cn('flex items-center justify-between gap-2 border-b border-[var(--separator)] px-2 py-1.5', className)}
...
function CodeCardIcon({ className, ...props }: CodiconProps) {
  return (
    <Codicon
      className={cn('shrink-0 text-[0.875rem] leading-none text-[var(--muted)]', className)}
...
function CodeCardSubtitle({ className, ...props }: React.ComponentProps<'span'>) {
  return (
    <span className={cn('font-normal text-[var(--muted)]', className)} data-slot="code-card-subtitle" {...props} />
  )
}
