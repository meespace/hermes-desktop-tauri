...
describe('resolveGatewayWsUrl', () => {
  describe('oauth mode', () => {
    ...
    it('passes the connection profile when minting', async () => {
      const getGatewayWsUrl = vi.fn().mockResolvedValue('ws://host/api/ws?ticket=fresh')

      await resolveGatewayWsUrl({ getGatewayWsUrl }, { ...oauthConn, profile: 'work' })
    ...
    it('throws a reauth error when the bridge cannot mint', async () => {
  ...
  describe('token / local mode', () => {
    ...
    it('falls back to the cached URL when the bridge method is absent', async () => {
      await expect(resolveGatewayWsUrl({}, tokenConn)).resolves.toBe(tokenConn.wsUrl)
    })

    it('treats a missing authMode as non-oauth', async () => {
...
describe('isGatewayReauthRequired', () => {
  ...
  it('detects plain objects tagged with needsOauthLogin', () => {
