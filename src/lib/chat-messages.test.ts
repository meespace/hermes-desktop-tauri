...
describe('toChatMessages', () => {
  ...
  it('keeps an assistant name from stored messages when present', () => {
    const [message] = toChatMessages([
      {
    ...
    expect(message?.assistantName).toBe('Planner')
  })
...
describe('preserveLocalAssistantErrors', () => {
  ...
  it('keeps a local assistant name when hydration reuses the same message id', () => {
    const nextMessages: ChatMessage[] = [
      {
        id: 'assistant-stream-1',
        parts: [{ text: 'hello', type: 'text' }],
    ...
    expect(merged[0]?.assistantName).toBe('Planner')
  })
})
