...
export interface GatewayClientOptions {
  ...
  socketFactory?: (url: string) => WebSocketLike | Promise<WebSocketLike>
...
export class JsonRpcGatewayClient {
  ...
  async connect(wsUrl: string): Promise<void> {
    ...
    console.info('[DEBUG-gateway] connect:start', wsUrl)

    const producedSocket = this.options.socketFactory?.(wsUrl)
    const socket =
      (producedSocket
        ? isPromiseLike(producedSocket)
          ? await producedSocket
          : producedSocket
        : null) ?? new WebSocket(wsUrl)
    this.socket = socket
    ...
    socket.addEventListener('close', () => {
      console.warn('[DEBUG-gateway] connect:close', {
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      const onOpen = () => {
        ...
        console.info('[DEBUG-gateway] connect:open', wsUrl)
        settled = true
        opened = true
        cleanup()
        this.setState('open')
      ...
      const onError = () => {
        console.error('[DEBUG-gateway] connect:error', {
          readyState: socket.readyState,
          url: wsUrl
        })
        this.setState('error')
      ...
      const onCloseBeforeOpen = (event?: { code?: number; reason?: string }) => {
        if (opened || settled) {
          return
        }

        console.error('[DEBUG-gateway] connect:close-before-open', {
          code: event?.code,
          reason: event?.reason,
          readyState: socket.readyState,
          url: wsUrl
        })
        const suffix = event?.reason ? `: ${event.reason}` : event?.code ? ` (${event.code})` : ''
        fail(new Error(`${this.options.closedErrorMessage}${suffix}`))
      }
      ...
      if (this.options.connectTimeoutMs > 0) {
        timer = setTimeout(() => {
          console.error('[DEBUG-gateway] connect:timeout', {
            readyState: socket.readyState,
            url: wsUrl
          })
          this.setState('error')
          try {
            ...
            // ignore close errors while timing out a failed connect attempt
          }
          fail(new Error(`${this.options.connectErrorMessage} (timed out)`))
