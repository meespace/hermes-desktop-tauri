...
export function toChatMessages(messages: SessionMessage[]): ChatMessage[] {
  ...
  const appendPartsToActiveAssistant = (parts: ChatMessagePart[], timestamp?: number): boolean => {
    ...
    active.assistantName ||= pendingToolAssistantName
  ...
  const flushPendingTools = (index: number) => {
    ...
    if (!appendPartsToActiveAssistant(pendingToolParts, pendingToolTimestamp)) {
      result.push({
        ...
        role: 'assistant',
        ...(pendingToolAssistantName ? { assistantName: pendingToolAssistantName } : {}),
  ...
  messages.forEach((message, index) => {
    ...
    const assistantName =
      message.role === 'assistant' && typeof message.name === 'string' && message.name.trim()
        ? resolveAssistantDisplayName(message.name)
        : undefined
    ...
    if (isToolOnlyAssistant) {
      ...
      pendingToolAssistantName ||= assistantName
      pendingToolTimestamp ??= message.timestamp
    ...
    if (message.role === 'assistant') {
      ...
      if (activeAssistant && (currentHasToolCall || activeHasToolCall)) {
        ...
        activeAssistant.assistantName ||= assistantName
...
export function preserveLocalAssistantErrors(
  ...
  const mergedNextMessages = nextMessages.map(message => {
    ...
    const withAssistantName =
      message.role === 'assistant' &&
      local?.role === 'assistant' &&
      !message.assistantName &&
      local.assistantName
        ? { ...message, assistantName: local.assistantName }
        : message

    if (withAssistantName.role !== 'assistant' || withAssistantName.error || withAssistantName.hidden) {
