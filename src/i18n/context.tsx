...
import { getHermesConfigRecord, saveHermesConfig, type HermesConfigRecord } from '@/hermes'
...
const defaultConfigClient: I18nConfigClient = {
  getConfig: async () => {
    ...
    const config = window.hermesDesktop.api ? await getHermesConfigRecord().catch(() => ({})) : {}
    const preferences = await window.hermesDesktop.settings?.getUiPreferences?.().catch(() => null)
    const localLanguage = normalizeLocale(preferences?.language)

    if (localLanguage === DEFAULT_LOCALE && !preferences?.language) {
      return config
    }

    return withConfigDisplayLanguage(config, localLanguage)
  },
  saveConfig: async config => {
    ...
    const language = normalizeLocale(getConfigDisplayLanguage(config))
    await window.hermesDesktop.settings?.setUiLanguage?.(language)

    if (!window.hermesDesktop.api) {
      return { ok: true }
    }

    return saveHermesConfig(config).catch(() => ({ ok: true }))
  }
}
