...
import { shouldShowCredentialWarning } from '@/lib/credential-warning'
...
function assistantNameForProfile(profile: null | string | undefined): string {
  return resolveAssistantDisplayName(profile || $activeGatewayProfile.get())
}
...
function applyRuntimeInfo(
  ...
  if (shouldShowCredentialWarning(info.credential_warning, info.provider, info.base_url)) {
...
export function useSessionActions({
  ...
  const createBackendSessionForSend = useCallback(async (preview: string | null = null): Promise<string | null> => {
    ...
    try {
      ...
      const assistantName = assistantNameForProfile(newChatProfile)
      ...
      updateSessionState(
        created.session_id,
        state => ({ ...state, ...(runtimeInfo ?? {}), assistantName: state.assistantName || assistantName }),
        stored
      )
  ...
  const resumeSession = useCallback(
    async (storedSessionId: string) => {
      ...
      const sessionProfile = stored?.profile
      const assistantName = assistantNameForProfile(sessionProfile)
      ...
      try {
        ...
        updateSessionState(
          ...
          state => ({
            ...state,
            ...(runtimeInfo ?? {}),
            assistantName: state.assistantName || assistantName,
        ...
        notifyError(err, translateNow('sessionFlow.resumeFailed'))
      } finally {
        if (isCurrentResume()) {
          busyRef.current = false
          setBusy(false)
  ...
  const branchCurrentSession = useCallback(
    async (messageId?: string): Promise<boolean> => {
      ...
      if (!sourceSessionId) {
        notify({
          kind: 'warning',
          title: translateNow('sessionFlow.branchUnavailableTitle'),
          message: translateNow('sessionFlow.branchUnavailable')
        })
      ...
      if (busyRef.current) {
        notify({
          kind: 'warning',
          title: translateNow('sessionFlow.sessionBusyTitle'),
          message: translateNow('sessionFlow.sessionBusy')
        })
      ...
      try {
        ...
        if (!branchMessages.length) {
          notify({
            kind: 'warning',
            title: translateNow('sessionFlow.branchNoTextTitle'),
            message: translateNow('sessionFlow.branchNoText')
          })
        ...
        const branched = await requestGateway<SessionCreateResponse>('session.create', {
          ...
          title: translateNow('sessionFlow.branch')
        })

        const routedSessionId = branched.stored_session_id ?? branched.session_id
        const assistantName =
          branchMessages.findLast(message => message.role === 'assistant')?.source.assistantName ||
          assistantNameForProfile(undefined)
        const preview = branchMessages.map(({ content }) => content).find(Boolean) ?? null

        setFreshDraftReady(false)
        upsertOptimisticSession(branched, routedSessionId, translateNow('sessionFlow.branch'), preview)
        ...
        notifyError(err, translateNow('sessionFlow.branchFailed'))
  ...
  const removeSession = useCallback(
    async (storedSessionId: string) => {
      ...
      try {
        ...
        notifyError(err, translateNow('sessionFlow.deleteFailed'))
      }
    },
    [
      activeSessionId,
  ...
  const archiveSession = useCallback(
    async (storedSessionId: string) => {
      ...
      const previousPinned = $pinnedSessionIds.get()
      const previousMessages = $messages.get()
      const previousUsage = $currentUsage.get()
      const previousCwd = $currentCwd.get()
      const previousBranch = $currentBranch.get()
      ...
      try {
        ...
        notify({ durationMs: 2_000, kind: 'success', message: translateNow('sessionActions.archived') })
      } catch (err) {
        if (archived) {
          setSessions(previousSessions)
          setSessionsTotal(previousSessionsTotal)
        ...
        notifyError(err, translateNow('sessionActions.archiveFailed'))
      }
    },
    [
      activeSessionId,
