    ...
    (
      ...
      const apply = () => {
        updateSessionState(sessionId, state => {
          ...
          const assistantName = state.assistantName || resolveAssistantDisplayName($activeGatewayProfile.get())
      ...
      const completedState = updateSessionState(sessionId, state => {
        ...
        const completeMessage = (message: ChatMessage): ChatMessage =>
          completionError
            ? {
                ...message,
                assistantName: message.assistantName || state.assistantName || resolveAssistantDisplayName($activeGatewayProfile.get()),
          ...
          ? prev.map(message =>
              message.id === streamId
                ? {
                    ...message,
                    assistantName:
                      message.assistantName || state.assistantName || resolveAssistantDisplayName($activeGatewayProfile.get()),
                    error,
                    pending: false
                  }
                : message
  ...
  const handleGatewayEvent = useCallback(
    (event: RpcEvent) => {
      ...
      if (event.type === 'gateway.ready') {
        ...
        if (shouldShowCredentialWarning(payload?.credential_warning, payload?.provider, payload?.base_url)) {
        ...
        setApprovalRequest({
          command: typeof promptPayload.command === 'string' ? promptPayload.command : '',
          description: typeof promptPayload.description === 'string' ? promptPayload.description : 'dangerous command',
          sessionId: sessionId ?? null
        })
        ...
        if (requestId) {
          setSecretRequest({
            requestId,
            envVar: typeof promptPayload.env_var === 'string' ? promptPayload.env_var : '',
            prompt: typeof promptPayload.prompt === 'string' ? promptPayload.prompt : '',
            sessionId: sessionId ?? null
          })
        }
