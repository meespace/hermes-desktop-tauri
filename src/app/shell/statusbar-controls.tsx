...
export function StatusbarControls({ className, leftItems = [], items = [], ...props }: StatusbarControlsProps) {
  ...
  return (
    <footer
      className={cn(
        'flex h-6 shrink-0 items-stretch justify-between gap-2 border-t border-[var(--separator)] bg-[color-mix(in_srgb,var(--surface)_90%,white_10%)] px-1.5 py-0 text-[var(--muted)] shadow-[inset_0_1px_0_color-mix(in_srgb,var(--surface)_92%,white_8%)] backdrop-blur-xl [-webkit-app-region:no-drag]',
        className
      )}
      {...props}
    >
...
function StatusbarItemView({ item, navigate }: { item: StatusbarItem; navigate: ReturnType<typeof useNavigate> }) {
  const content = (
    <>
      ...
      {item.detail && <span className="truncate text-[color-mix(in_srgb,var(--foreground)_60%,transparent)]">{item.detail}</span>}
    </>
  )
    ...
    return (
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <button
            className={cn(
              'inline-flex h-full cursor-pointer items-center gap-1 rounded-[calc(var(--radius)*1.15)] border border-transparent px-1.5 text-[0.6875rem] text-[var(--muted)] transition-[background-color,border-color,color,box-shadow] hover:border-[var(--border)] hover:bg-[var(--surface-secondary)] hover:text-[var(--foreground)] disabled:cursor-default disabled:opacity-45',
              item.className
            )}
        ...
        >
          {item.menuContent
            ...
            : (item.menuItems ?? [])
                ...
                .map(menuItem => (
                  <DropdownMenuItem
                    className={cn('gap-2 text-foreground focus:bg-[var(--surface-secondary)] [&_svg]:size-4', menuItem.className)}
  ...
  if (item.variant === 'text' && !item.onSelect && !item.to && !item.href) {
    return (
      <div
        className={cn(
          'inline-flex h-full items-center gap-1 px-1.5 text-[0.6875rem] text-[var(--muted)]',
          item.className
        )}
      >
        {content}
