...
export function GatewayMenuPanel({
  ...
  return (
    <div className="text-sm">
      <div className="flex items-center justify-between gap-2 px-3 py-2.5">
        ...
        <div className="flex items-center">
          <Button
            ...
            className="size-7 text-[var(--muted)] hover:text-foreground"
      ...
      <div className="border-t border-[var(--separator)] px-3 py-2 text-xs text-[var(--muted)]">
        ...
        <div className="border-t border-[var(--separator)] px-3 py-2">
          ...
          <ul className="mt-1.5 space-y-0.5">
            {recentLogs.map((line, index) => (
              <li
                className="truncate font-mono text-[0.68rem] text-[color-mix(in_srgb,var(--foreground)_70%,transparent)]"
                key={`${index}:${line}`}
                title={line.trim()}
              >
                {trimLogLine(line) || '\u00A0'}
          ...
          <button
            className="mt-1.5 text-[0.66rem] font-medium text-[var(--muted)] transition hover:text-[var(--foreground)]"
            onClick={onOpenSystem}
            type="button"
          >
            View all logs →
...
function SectionLabel({ children }: { children: string }) {
  return <div className="text-[0.62rem] font-semibold uppercase tracking-[0.14em] text-[var(--muted)]">{children}</div>
}
