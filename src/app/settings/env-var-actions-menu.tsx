...
export function EnvVarActionsMenu({
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align={align} aria-label={`Actions for ${label}`} className="w-44" sideOffset={sideOffset}>
        {hasDocs ? (
          <DropdownMenuItem
            onSelect={event => {
...
export function EnvVarActionsTrigger({ className, label, ...props }: EnvVarActionsTriggerProps) {
  return (
    <Button
      ...
      className={cn('text-[var(--muted)] hover:text-foreground', className)}
      size="icon-xs"
