...
const PILL_TONE: Record<'good' | 'muted' | 'warn' | 'bad', string> = {
  good: 'bg-[var(--success-soft)] text-[var(--success-soft-foreground)] shadow-[var(--field-shadow)]',
  muted: 'bg-[var(--default-soft)] text-[var(--default-soft-foreground)] shadow-[var(--field-shadow)]',
...
function jobTitle(job: CronJob, fallback = 'Cron job'): string {
...
function dayName(value: string, copy: Translations['cron']): string {
  return copy.scheduleSummary.days[value] ?? copy.scheduleSummary.day(value)
}
...
function scheduleSummary(option: ScheduleOption, expr: string, copy: Translations['cron']): string {
  const parts = cronParts(expr)

  if (!parts) {
    return copy.scheduleOptions[option.value]?.hint ?? option.hint
  }

  const [minute, hour, dayOfMonth, , dayOfWeek] = parts

  if (option.value === 'daily') {
    return copy.scheduleSummary.daily(formatCronTime(minute, hour))
  }

  if (option.value === 'weekdays') {
    return copy.scheduleSummary.weekdays(formatCronTime(minute, hour))
  }

  if (option.value === 'weekly') {
    return copy.scheduleSummary.weekly(dayName(dayOfWeek, copy), formatCronTime(minute, hour))
  }

  if (option.value === 'monthly') {
    return copy.scheduleSummary.monthly(dayOfMonth, formatCronTime(minute, hour))
  }

  if (option.value === 'hourly') {
    return minute === '0' ? copy.scheduleSummary.hourlyTop : copy.scheduleSummary.hourlyAt(minute.padStart(2, '0'))
  }
...
export function CronView({ setStatusbarItemGroup: _setStatusbarItemGroup, ...props }: CronViewProps) {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      notifyError(err, t.cron.loadFailed)
    } finally {
      setRefreshing(false)
    }
  }, [t.cron.loadFailed])
  ...
  const visibleJobs = useMemo(() => {
    ...
    return jobs
      ...
      .sort((a, b) => jobTitle(a, t.cron.fallbackTitle).localeCompare(jobTitle(b, t.cron.fallbackTitle)))
  }, [jobs, query, t.cron.fallbackTitle])
  ...
  async function handlePauseResume(job: CronJob) {
    ...
    try {
      ...
      notify({
        kind: 'success',
        title: isPaused ? t.cron.resumed : t.cron.paused,
        message: truncate(jobTitle(job, t.cron.fallbackTitle), 60)
      })
    } catch (err) {
      notifyError(err, t.cron.updateFailed)
    } finally {
      setBusyJobId(null)
    }
  }
  ...
  async function handleTrigger(job: CronJob) {
    ...
    try {
      ...
      setJobs(current => (current ? current.map(row => (row.id === job.id ? updated : row)) : current))
      notify({ kind: 'success', title: t.cron.triggered, message: truncate(jobTitle(job, t.cron.fallbackTitle), 60) })
    } catch (err) {
      notifyError(err, t.cron.triggerFailed)
    } finally {
      setBusyJobId(null)
    }
  }
  ...
  async function handleConfirmDelete() {
    ...
    try {
      ...
      notify({ kind: 'success', title: t.cron.deleted, message: truncate(jobTitle(pendingDelete, t.cron.fallbackTitle), 60) })
      setPendingDelete(null)
    } catch (err) {
      notifyError(err, t.cron.deleteFailed)
    } finally {
      setDeleting(false)
    }
  }
  ...
  async function handleEditorSave(values: EditorValues) {
    if (editor.mode === 'create') {
      ...
      notify({ kind: 'success', title: t.cron.created, message: truncate(jobTitle(created, t.cron.fallbackTitle), 60) })
      ...
      notify({ kind: 'success', title: t.cron.updated, message: truncate(jobTitle(updated, t.cron.fallbackTitle), 60) })
    }
  ...
  return (
    <PageSearchShell
      ...
      searchPlaceholder={t.cron.searchPlaceholder}
    ...
    >
      {!jobs ? (
        ...
        <EmptyState
          actionLabel={totalCount === 0 ? t.cron.createFirst : undefined}
          description={totalCount === 0 ? t.cron.emptyDescription : t.cron.noMatchesDescription}
          onAction={totalCount === 0 ? () => setEditor({ mode: 'create' }) : undefined}
          title={totalCount === 0 ? t.cron.emptyTitle : t.cron.noMatchesTitle}
        />
      ) : (
        <div className="h-full overflow-y-auto px-4 py-3">
          ...
          <div className="mb-2 flex items-center justify-between">
            <span className="rounded-full bg-[var(--default-soft)] px-2.5 py-1 text-[0.68rem] font-medium uppercase tracking-[0.12em] text-[var(--default-soft-foreground)] shadow-[var(--field-shadow)]">
              {t.cron.activeSummary(enabledCount, totalCount)}
            </span>
          ...
          <div className={cn(CRON_SURFACE_CLASS, 'divide-y divide-[var(--separator)] overflow-hidden')}>
            {visibleJobs.map(job => (
              <CronJobRow
      ...
      <Dialog onOpenChange={open => !open && !deleting && setPendingDelete(null)} open={pendingDelete !== null}>
        <DialogContent className="max-w-md border-[var(--border)] bg-[var(--overlay)] shadow-[var(--overlay-shadow)]">
          <DialogHeader>
            <DialogTitle>{t.cron.deleteTitle}</DialogTitle>
            <DialogDescription>
              {pendingDelete ? (
                t.cron.deleteDescription(truncate(jobTitle(pendingDelete, t.cron.fallbackTitle), 60))
              ) : null}
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            ...
            <Button disabled={deleting} onClick={() => void handleConfirmDelete()} variant="destructive">
              {deleting ? t.cron.deleting : t.common.delete}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
...
function CronJobRow({
  ...
  return (
    <div className="grid gap-3 px-3 py-3 transition-colors hover:bg-[var(--surface-secondary)] sm:grid-cols-[minmax(0,1fr)_auto] sm:items-start">
      <button
        className="min-w-0 cursor-pointer rounded-[calc(var(--radius)*1.25)] text-left transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[color-mix(in_srgb,var(--focus)_22%,transparent)]"
        onClick={onEdit}
        type="button"
      >
        <div className="flex flex-wrap items-center gap-2">
          <span className="truncate text-sm font-medium">{jobTitle(job, copy.fallbackTitle)}</span>
          <StatePill tone={STATE_TONE[state] ?? 'muted'}>{copy.states[state] ?? state}</StatePill>
          {deliver && deliver !== DEFAULT_DELIVER && <StatePill tone="muted">{copy.delivery[deliver] ?? deliver}</StatePill>}
        </div>
        {hasName && prompt && <p className="mt-1 truncate text-xs text-[var(--muted)]">{truncate(prompt, 120)}</p>}
        <div className="mt-1 flex flex-wrap items-center gap-x-4 gap-y-1 text-[0.68rem] text-[var(--muted)]">
          <span className="inline-flex items-center gap-1 font-mono">
            <Clock className="size-3" />
            {jobScheduleDisplay(job)}
          </span>
          <span>{copy.last}: {formatTime(job.last_run_at)}</span>
          <span>{copy.next}: {formatTime(job.next_run_at)}</span>
        </div>
        {job.last_error && (
      ...
      <div className="flex shrink-0 items-center gap-0.5">
        <IconAction
          ...
          title={isPaused ? copy.resume : copy.pause}
        >
          {isPaused ? <Play className="size-3.5" /> : <Pause className="size-3.5" />}
        </IconAction>
        <IconAction aria-label={copy.triggerNow} disabled={busy} onClick={onTrigger} title={copy.triggerNow}>
          <Zap className="size-3.5" />
        </IconAction>
        <IconAction aria-label={copy.edit} onClick={onEdit} title={copy.edit}>
...
function IconAction({ children, className, ...props }: Omit<React.ComponentProps<typeof Button>, 'size' | 'variant'>) {
  return (
    <Button
      className={cn('size-7 rounded-[calc(var(--radius)*1.25)] text-[var(--muted)] hover:text-foreground', className)}
      size="icon"
      variant="ghost"
      {...props}
    >
...
function EmptyState({
  ...
  return (
    <div className="grid h-full place-items-center px-6 py-12 text-center">
      <div className={cn(CRON_SURFACE_CLASS, 'max-w-sm space-y-2 px-6 py-6')}>
...
function CronEditorDialog({
  ...
  const scheduleHint = scheduleSummary(selectedScheduleOption, schedule, t.cron)
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    if (!trimmedPrompt || !trimmedSchedule) {
      setError(t.cron.promptScheduleRequired)
  ...
  return (
    <Dialog onOpenChange={value => !value && !saving && onClose()} open={open}>
      <DialogContent className="max-w-lg border-[var(--border)] bg-[var(--overlay)] shadow-[var(--overlay-shadow)]">
        <DialogHeader>
          <DialogTitle>{isEdit ? t.cron.editTitle : t.cron.createTitle}</DialogTitle>
          <DialogDescription>
            {isEdit ? t.cron.editDescription : t.cron.createDescription}
          </DialogDescription>
        </DialogHeader>

        <form className="grid gap-4" onSubmit={handleSubmit}>
          <Field htmlFor="cron-name" label={t.cron.name} optionalText={t.cron.optional}>
            <Input
              autoFocus
              id="cron-name"
          ...
          <Field htmlFor="cron-prompt" label={t.cron.prompt}>
            <Textarea
          ...
          <div className="grid items-start gap-4 sm:grid-cols-2">
            <Field htmlFor="cron-frequency" label={t.cron.frequency}>
                ...
                <SelectContent>
                  {SCHEDULE_OPTIONS.map(option => (
                    <SelectItem key={option.value} value={option.value}>
                      {t.cron.scheduleOptions[option.value]?.label ?? option.label}
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
            ...
            <Field htmlFor="cron-deliver" label={t.cron.deliverTo}>
              <Select onValueChange={setDeliver} value={deliver}>
                ...
                <SelectContent>
                  {DELIVERY_OPTIONS.map(option => (
                    <SelectItem key={option.value} value={option.value}>
                      {t.cron.delivery[option.value] ?? option.label}
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
          ...
          {schedulePreset === 'custom' ? (
            <Field htmlFor="cron-schedule" label={t.cron.customSchedule}>
              <Input
                className="font-mono"
                id="cron-schedule"
                onChange={event => setSchedule(event.target.value)}
                placeholder={t.cron.customSchedulePlaceholder}
                value={schedule}
              />
              <FieldHint>{t.cron.customScheduleHint}</FieldHint>
            </Field>
          ) : (
            <div className="rounded-[calc(var(--radius)*1.5)] border border-[var(--border)] bg-[var(--surface-secondary)] px-3 py-2 shadow-[var(--field-shadow)]">
              <div className="flex flex-wrap items-center justify-between gap-2 text-xs">
                <span className="font-medium text-foreground">{scheduleHint}</span>
                <span className="font-mono text-[var(--muted)]">{schedule}</span>
              </div>
            </div>
          )}

          {error && (
            <div className="flex items-start gap-2 rounded-[calc(var(--radius)*1.5)] border border-[var(--danger-soft)] bg-[var(--danger-soft)] px-3 py-2 text-xs text-[var(--danger-soft-foreground)]">
          ...
          <DialogFooter>
            ...
            <Button disabled={saving} type="submit">
              {saving ? t.common.saving : isEdit ? t.cron.saveChanges : t.cron.createCron}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
  ...
  return (
    <div className="grid gap-1.5">
      <label className="flex items-baseline gap-2 text-xs font-medium text-foreground" htmlFor={htmlFor}>
        {label}
        {optionalText && <span className="text-[0.65rem] font-normal text-[var(--muted)]">{optionalText}</span>}
      </label>
      {children}
    </div>
  )
...
function FieldHint({ children }: { children: React.ReactNode }) {
  return <p className="text-[0.66rem] leading-4 text-[var(--muted)]">{children}</p>
}
