...
function statusGlyph(status: SubagentStatus): ReactNode {
  if (status === 'running' || status === 'queued') {
    return (
      <BrailleSpinner
        ariaLabel="Running"
        className="size-3.5 shrink-0 text-[0.95rem] text-[var(--muted)]"
        spinner="breathe"
      />
    )
  }
...
const STREAM_TONE: Record<SubagentStreamEntry['kind'], string> = {
  progress: 'text-[color-mix(in_srgb,var(--foreground)_64%,transparent)]',
  summary: 'text-foreground/85',
  thinking: 'text-[color-mix(in_srgb,var(--foreground)_62%,transparent)]',
...
function streamGlyph(entry: SubagentStreamEntry): ReactNode {
  ...
  if (entry.kind === 'thinking') {
    return (
      <span aria-hidden className="font-mono text-[0.7rem] leading-none text-[color-mix(in_srgb,var(--foreground)_56%,transparent)]">
        …
      </span>
    )
  }
  ...
  return (
    ...
    >
      <header className="mb-3 shrink-0">
        ...
        <p className="text-xs text-[color-mix(in_srgb,var(--foreground)_64%,transparent)]">
...
function SubagentTree({ tree }: { tree: SubagentNode[] }) {
  ...
  if (tree.length === 0) {
    return (
      <div className="grid place-items-center gap-3 py-12 text-center">
        <Sparkles className="size-6 text-[var(--muted)]" />
        <p className="text-sm font-medium text-foreground/90">No live subagents</p>
        <p className="max-w-md text-xs leading-relaxed text-[color-mix(in_srgb,var(--foreground)_62%,transparent)]">
  ...
  return (
    <div className="flex min-h-0 min-w-0 flex-1 flex-col gap-4 overflow-hidden">
      <p className="shrink-0 text-[0.7rem] text-[color-mix(in_srgb,var(--foreground)_58%,transparent)]">
        {summary.join(' · ')}
      </p>
...
function DelegationGroup({ group, nowMs }: { group: RootGroup; nowMs: number }) {
  ...
  return (
    <section className="grid min-w-0 gap-3">
      <p className="text-[0.66rem] font-medium uppercase tracking-wider text-[color-mix(in_srgb,var(--foreground)_58%,transparent)]">
        {group.label}{' '}
        <span className="text-[color-mix(in_srgb,var(--foreground)_42%,transparent)]">·</span> {group.nodes.length}{' '}
        workers
    ...
    <div className="flex min-w-0 items-baseline gap-2 text-[0.72rem] leading-relaxed" ref={enterRef}>
      ...
      <span className={cn('min-w-0 flex-1 wrap-anywhere', tone, isMono && 'font-mono text-[0.69rem]')}>
        ...
        {active ? (
          <BrailleSpinner
            ariaLabel="Streaming"
            className="ml-1 inline-block size-2.5 align-middle text-[color-mix(in_srgb,var(--foreground)_56%,transparent)]"
            spinner="breathe"
          />
        ) : null}
      </span>
    ...
    <div className={cn('grid min-w-0 max-w-full gap-2', depth > 0 && 'pl-4')} data-slot="tool-block" ref={enterRef}>
      ...
      >
        ...
        <span className="flex min-w-0 flex-1 flex-col gap-0.5">
          ...
          {subtitle.length > 0 ? (
            <FadeText className="text-[0.66rem] leading-[1.05rem] text-[color-mix(in_srgb,var(--foreground)_56%,transparent)]">
              {subtitle.join(' · ')}
            </FadeText>
          ) : null}
        </span>
      ...
      {open && fileLines.length > 0 ? (
        <div className="grid min-w-0 gap-0.5 pl-6">
          <p className="text-[0.58rem] font-medium uppercase tracking-wider text-[color-mix(in_srgb,var(--foreground)_52%,transparent)]">
            Files
          </p>
          {fileLines.slice(0, 8).map(line => (
            <p
              className="wrap-break-word font-mono text-[0.67rem] leading-relaxed text-[color-mix(in_srgb,var(--foreground)_66%,transparent)]"
              key={line}
            >
              {line}
            </p>
          ...
          {fileLines.length > 8 ? (
            <p className="font-mono text-[0.67rem] leading-relaxed text-[color-mix(in_srgb,var(--foreground)_54%,transparent)]">
