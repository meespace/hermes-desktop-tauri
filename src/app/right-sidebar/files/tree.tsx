...
function TreeSizingState() {
  ...
  return (
    <div className="flex h-full min-h-24 items-center justify-center px-3 text-[0.68rem] text-[var(--muted)]">
...
function ProjectTreeRow({
  ...
  return (
    <div
      ...
      className={cn(
        'group/row flex h-full cursor-pointer select-none items-center gap-1 rounded-[calc(var(--radius)*1.25)] border border-transparent px-3 text-xs font-normal leading-[var(--file-tree-row-height)] text-[color-mix(in_srgb,var(--foreground)_76%,transparent)] transition-[background-color,border-color,color,box-shadow] hover:border-[var(--border)] hover:bg-[var(--surface-secondary)] hover:text-[var(--foreground)]',
        node.isSelected && 'border-[var(--border)] bg-[var(--surface-secondary)] text-[var(--foreground)] shadow-[var(--field-shadow)]',
        isPlaceholder && 'pointer-events-none italic text-[var(--muted)]'
      )}
    ...
    >
      ...
      <span aria-hidden className="flex w-3.5 items-center justify-center text-[var(--muted)]">
        {isPlaceholder ? (
