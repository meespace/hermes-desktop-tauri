...
function loadErrorTitle(error: PreviewLoadErrorState): string {
  ...
  if (description.includes('module script') || description.includes('mime type')) {
    return translateNow('preview.appFailedToBoot')
  }

  if (description.includes('connection') || description.includes('refused') || description.includes('not found')) {
    return translateNow('preview.serverNotFound')
  }

  return translateNow('preview.failedToLoad')
}
  ...
  return (
    <PreviewEmptyState
      body={
        <>
          <a
            className="pointer-events-auto block cursor-pointer font-mono text-[var(--muted)] underline decoration-current/20 underline-offset-4 transition-colors hover:text-foreground"
            href={error.url}
            onClick={event => {
          ...
          <div className="mt-1 text-[0.6875rem] text-[color-mix(in_srgb,var(--foreground)_62%,transparent)]">
            {error.description}
          </div>
        </>
      }
      consoleHeight={consoleHeight}
      primaryAction={{ label: t.preview.tryAgain, onClick: onRetry }}
      secondaryAction={
        onRestartServer
          ? {
              disabled: restarting,
              label: restarting ? t.preview.hermesRestarting : t.preview.askRestartServer,
...
export function PreviewPane({
  ...
  const restartServer = useCallback(async () => {
    ...
    try {
      ...
      notify({
        kind: 'info',
        title: t.preview.restartingServer,
        message: t.preview.restartingMessage,
        durationMs: 4000
      })
    } catch (error) {
      appendConsoleEntry({
      ...
      notifyError(error, t.preview.restartFailed)
    }
  }, [appendConsoleEntry, consoleState, currentUrl, onRestartServer, t.preview.restartFailed, t.preview.restartingMessage, t.preview.restartingServer])
  ...
  useEffect(() => {
    ...
    if (previewServerRestart.status === 'complete') {
      ...
      notify({
        kind: 'success',
        title: t.preview.serverRestarted,
        message: previewServerRestart.message?.slice(0, 160) || t.preview.serverRestartedMessage,
        durationMs: 3500
      })
    } else if (previewServerRestart.status === 'error') {
      notify({
        kind: 'warning',
        title: t.preview.restartFailedTitle,
        message: previewServerRestart.message?.slice(0, 200) || t.preview.restartFailedMessage,
        durationMs: 6000
      })
    }
  ...
  useEffect(() => {
    ...
    appendConsoleEntry({
      level: 1,
      message: t.preview.workspaceChangedReloading
    })
    reloadPreview()
  }, [appendConsoleEntry, reloadPreview, reloadRequest, t.preview.workspaceChangedReloading, target.kind])
  ...
  return (
    <aside className="relative flex h-full w-full min-w-0 flex-col overflow-hidden bg-transparent text-[var(--muted)]">
      <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
        {!embedded && (
          <div className="pointer-events-none flex min-h-(--titlebar-height) items-center gap-1.5 border-b border-[var(--separator)] bg-[var(--surface)]/92 px-2 py-1 backdrop-blur-xl">
