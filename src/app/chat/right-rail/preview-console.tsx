...
function ConsoleRow({ copyText, log, onSend, onToggleSelect, selected }: ConsoleRowProps) {
  return (
    <div
      className={cn(
        'group/row grid grid-cols-[3.25rem_minmax(0,1fr)_auto] items-start gap-2 rounded-[calc(var(--radius)*1.25)] border border-transparent px-1 py-1 transition-[background-color,border-color,color,box-shadow] hover:bg-[var(--surface-secondary)]',
        selected && 'border-[var(--border)] bg-[var(--surface-secondary)] shadow-[var(--field-shadow)]'
      )}
    >
      <button
        className={cn(
      ...
      <div className="min-w-0" data-selectable-text="true">
        ...
        {log.source && (
          <span className="block truncate text-[color-mix(in_srgb,var(--foreground)_54%,transparent)]">
      ...
      <span className="opacity-0 transition-opacity group-hover/row:opacity-100">
        <CopyButton
          appearance="inline"
          className="rounded-[calc(var(--radius)*1.15)] p-1 text-[var(--muted)] transition-colors hover:bg-[var(--surface-secondary)] hover:text-foreground"
...
export function PreviewConsolePanel({
  ...
  return (
    <div
      className="pointer-events-auto absolute inset-x-0 bottom-0 z-20 flex h-(--preview-console-height) min-h-8 flex-col overflow-hidden border-t border-[var(--separator)] bg-[var(--surface)]/96 backdrop-blur-xl"
    ...
    >
      <div
        ...
        role="separator"
      >
        <span className="absolute left-1/2 top-1/2 h-0.75 w-23 -translate-x-1/2 -translate-y-1/2 rounded-full bg-[color-mix(in_srgb,var(--foreground)_36%,transparent)] opacity-0 transition-opacity duration-100 group-hover:opacity-[0.5]" />
      </div>
      <div className="flex h-8 shrink-0 items-center justify-between border-b border-[var(--separator)] px-2">
        <div className="flex items-center gap-2 text-[0.6875rem] font-medium text-[var(--muted)]">
          <PanelBottom className="size-3.5" />
          Preview Console
          {selectedLogIds.size > 0 && (
            <span className="rounded-full bg-[var(--surface-secondary)] px-1.5 py-px text-[0.5625rem] text-[var(--muted)] shadow-[var(--field-shadow)]">
        ...
        <div className="flex items-center gap-1">
          <button
            className="inline-flex items-center gap-1 rounded-[calc(var(--radius)*1.15)] px-1.5 py-0.5 text-[0.625rem] text-[var(--muted)] transition-colors hover:bg-[var(--surface-secondary)] hover:text-foreground disabled:opacity-40"
      ...
      >
        {logs.length > 0 ? (
          ...
          <div className="py-2 text-[color-mix(in_srgb,var(--foreground)_62%,transparent)]">No console messages yet.</div>
        )}
      </div>
    </div>
  )
