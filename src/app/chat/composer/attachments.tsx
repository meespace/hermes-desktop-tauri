  ...
  return (
    ...
    >
      <button
        ...
        className="flex max-w-56 items-center gap-2 rounded-[calc(var(--radius)*1.5)] border border-[var(--border)] bg-[var(--surface)] px-2 py-1.5 text-left shadow-[var(--field-shadow)] transition-[background-color,border-color,color,box-shadow] hover:border-[var(--accent-soft)] hover:bg-[var(--surface-secondary)] disabled:cursor-default"
        disabled={!canPreview}
      ...
      >
        {attachment.previewUrl && attachment.kind === 'image' ? (
          <img
            ...
            className="size-8 shrink-0 rounded-[calc(var(--radius)*1.1)] border border-[var(--border)] object-cover"
            draggable={false}
            src={attachment.previewUrl}
          />
        ) : (
          <span className="grid size-8 shrink-0 place-items-center rounded-[calc(var(--radius)*1.1)] border border-[var(--border)] bg-[var(--surface-secondary)] text-[var(--muted)]">
        ...
        <span className="min-w-0">
          ...
          {detail && (
            <span className="block truncate font-mono text-[0.6rem] leading-3 text-[color-mix(in_srgb,var(--foreground)_54%,transparent)]">
              {detail}
            </span>
          )}
        </span>
      ...
      {onRemove && (
        <button
          ...
          className="absolute -right-1 -top-1 grid size-4 place-items-center rounded-full border border-[var(--border)] bg-[var(--surface)] text-[var(--muted)] opacity-0 shadow-[var(--field-shadow)] transition-[opacity,background-color,color] hover:bg-[var(--surface-secondary)] hover:text-foreground group-hover/attachment:opacity-100 focus-visible:opacity-100"
