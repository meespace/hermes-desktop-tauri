    ...
    >
      {items.length === 0 ? (
        ...
        items.map((item, index) => {
          ...
          return (
            <button
              className={cn(COMPLETION_DRAWER_ROW_CLASS, index === activeIndex && 'bg-[var(--surface-secondary)]')}
              ...
              type="button"
            >
              <span className="grid size-3.5 shrink-0 place-items-center text-[var(--muted)]">
              ...
              {description && (
                <span className="min-w-0 flex-1 truncate leading-5 text-[var(--muted)]">{description}</span>
              )}
            </button>
          )
        })
