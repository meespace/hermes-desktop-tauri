...
export function VoiceActivity({ state }: { state: VoiceActivityState }) {
  ...
  const title = recording ? t.composer.voice.dictating : t.composer.voice.transcribing

  return (
    <div
      aria-live="polite"
      className={cn(
        'flex h-8 items-center gap-2 rounded-xl border border-[var(--border)] bg-[var(--surface-secondary)]/72 px-2.5 text-xs text-[var(--muted)] shadow-[var(--field-shadow)]',
...
export function VoicePlaybackActivity() {
  ...
  const title = preparing
    ? t.composer.voice.preparingAudio
    : playback.source === 'voice-conversation'
      ? t.composer.voice.speakingResponse
      : t.composer.voice.readingAloud
