...
export function microphoneError(error: unknown): Error {
  ...
  if (name === 'NotAllowedError' || name === 'SecurityError') {
    return new Error(translateNow('composer.voice.permissionDenied'))
  }

  if (name === 'NotFoundError' || name === 'DevicesNotFoundError') {
    return new Error(translateNow('composer.voice.noMicrophone'))
  }

  if (name === 'NotReadableError' || name === 'TrackStartError') {
    return new Error(translateNow('composer.voice.microphoneInUse'))
  }

  if (name === 'OverconstrainedError') {
    return new Error(translateNow('composer.voice.constraintsUnsupported'))
  }
  ...
  return new Error(translateNow('composer.voice.startRecordingFailed'))
}
...
export function useMicRecorder(): { handle: MicRecorderHandle; level: number; recording: boolean } {
  ...
  const start: MicRecorderHandle['start'] = async (options = {}) => {
    ...
    if (!navigator.mediaDevices?.getUserMedia || typeof MediaRecorder === 'undefined') {
      throw new Error(translateNow('composer.voice.unsupportedRuntime'))
    }

    const permitted = await window.hermesDesktop?.requestMicrophoneAccess?.()

    if (permitted === false) {
      throw new Error(translateNow('composer.voice.permissionDenied'))
    }
    ...
    recorder.onerror = event => {
      const error = microphoneError((event as Event & { error?: unknown }).error)
      const resolver = stopResolverRef.current
      stopResolverRef.current = null
      cleanup()
