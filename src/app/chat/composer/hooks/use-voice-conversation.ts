...
export function useVoiceConversation({
  ...
  const startListening = useCallback(async () => {
    ...
    try {
      ...
      await handle.start({
        ...
        onError: error => {
          notifyError(error, translateNow('composer.voice.microphoneFailed'))
          pendingStartRef.current = false
          onFatalError?.()
        },
      ...
      notifyError(error, translateNow('composer.voice.startSessionFailed'))
      pendingStartRef.current = false
      setStatus('idle')
      onFatalError?.()
    }
  ...
  const speak = useCallback(async (text: string) => {
    ...
    try {
      ...
      notifyError(error, translateNow('composer.voice.playbackFailed'))
    } finally {
      if (enabledRef.current) {
        pendingStartRef.current = true
        setStatus('idle')
  ...
  const start = useCallback(async () => {
    if (!onTranscribeAudio) {
      notify({
        ...
        message: translateNow('composer.voice.configureStt')
      })
      onFatalError?.()
