...
/**
 * Full-bleed affordance shown while files or an in-app session are dragged over
 * the chat area. Always `pointer-events-none` so the drop lands on the real
 * element underneath and the drop-zone handler claims it.
 */
export function ChatDropOverlay({ kind }: { kind: DragKind }) {
  ...
  const icon = resolvedKind === 'session' ? 'comment-discussion' : 'cloud-upload'
  const label = resolvedKind === 'session' ? 'Drop session to reference' : 'Drop files to attach'
  ...
  return (
    ...
    >
      ...
      <div className="relative flex items-center gap-2 rounded-full border border-[color-mix(in_srgb,var(--dt-composer-ring)_45%,transparent)] bg-[color-mix(in_srgb,var(--dt-card)_92%,transparent)] px-4 py-2 text-[0.8125rem] font-medium text-foreground shadow-composer">
        <Codicon className="text-[var(--accent)]" name={icon} size="1rem" />
        {label}
      </div>
    </div>
  )
