...
import { ChatSwapOverlay } from './chat/chat-swap-overlay'
...
const KeybindPanel = lazy(async () => ({ default: (await import('./shell/keybind-panel')).KeybindPanel }))
...
export function DesktopController() {
  ...
  const refreshCronSessions = useCallback(async () => {
    try {
      ...
      // Non-fatal: pinned cron runs simply stay unresolved until the next successful refresh.
    }
  }, [])
  ...
  const refreshCronJobs = useCallback(async () => {
    try {
      ...
      // Non-fatal: the sidebar keeps the last-known jobs.
    }
  }, [])
  ...
  }, [profileScope, refreshCronJobs, refreshCronSessions])
  ...
  const loadMoreSessionsForProfile = useCallback(async (profile: string) => {
    ...
    const inKey = (session: SessionInfo) => normalizeProfileKey(session.profile) === key
    ...
    setSessions(prev => [...prev.filter(session => !inKey(session)), ...mergeSessionPage(prev.filter(inKey), result.sessions, keep)])
  ...
  const hydrateFromStoredSession = useCallback(
    async (
      ...
      for (let index = 0; index < Math.max(1, attempts); index += 1) {
        try {
          const profile = $sessions.get().find(session => session.id === storedSessionId)?.profile
  ...
  const takeoverTerminalView = (
    <div className="relative flex h-full min-h-0 min-w-0 flex-col overflow-hidden bg-[var(--background)]">
      <TerminalSlot />
    </div>
  )
