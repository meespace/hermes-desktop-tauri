import { JsonRpcGatewayClient } from '@/lib/shared'
import { createTauriWebSocket, isTauriRuntime } from '@/lib/tauri-websocket'
...
const LOCAL_MODEL_CONTEXT_LENGTH = 8192

// Profile that profile-scoped REST settings should target. Tauri passes this
// through to the native proxy so remote profile overrides can resolve their own
// backend connection. Null keeps the primary/default connection path.
...
function api<T>(request: HermesApiRequest): Promise<T> {
  return window.hermesDesktop.api<T>({ ...request, ...profileScoped() })
}
...
export class HermesGateway extends JsonRpcGatewayClient {
  constructor() {
    super({
      ...
      socketFactory: url => (isTauriRuntime() ? createTauriWebSocket(url) : new WebSocket(url))
    })
  }
}
...
export function getProfiles(): Promise<ProfilesResponse> {
  // Profile discovery is global. If this goes through the scoped api() helper,
  // the active profile backend can report only itself, which makes the sidebar
  // rail collapse to just the default profile.
...
export function setGlobalModel(
  ...
  return setModelAssignment({ scope: 'main', provider, model }).then(result => {
    return {
      ok: result.ok,
...
function desktopModelContextLengthForAssignment(body: ModelAssignmentRequest): number | null {
  if (body.scope !== 'main') {
    return null
  }

  if (isLocalModelBaseUrl(body.base_url) || isLocalModelProvider(body.provider)) {
    return LOCAL_MODEL_CONTEXT_LENGTH
  }

  // Restore official auto-detection for built-in providers and explicit remote
  // custom endpoints. For custom without a base_url, keep the existing config
  // because Desktop cannot safely tell whether it is local or remote.
...
async function applyDesktopModelContextLength(body: ModelAssignmentRequest): Promise<HermesConfigRecord | null> {
  const nextContextLength = desktopModelContextLengthForAssignment(body)
  if (nextContextLength === null) {
    return null
  }

  const previousConfig = await getHermesConfigRecord()
  if (previousConfig.model_context_length === nextContextLength) {
    return null
  }

  await saveHermesConfig({
    ...previousConfig,
    model_context_length: nextContextLength
  })

  return previousConfig
}

export async function setModelAssignment(body: ModelAssignmentRequest): Promise<ModelAssignmentResponse> {
  let rollbackConfig: HermesConfigRecord | null = null

  try {
    rollbackConfig = await applyDesktopModelContextLength(body)
  } catch {
    // If the context guard cannot be applied, continue with the model switch.
  }

  try {
    return await window.hermesDesktop.api<ModelAssignmentResponse>({
      path: '/api/model/set',
      method: 'POST',
      body
    })
    ...
    if (rollbackConfig) {
      await saveHermesConfig(rollbackConfig).catch(() => undefined)
    }
    throw error
  }
}
